...
export const LocalVideoComponent = ({ stream, isVideoOff, isMuted, isSpeaking, isFilmstrip = false }: any) => {
    ...
    <div className={`relative group ${isFilmstrip ? 'rounded-xl' : 'rounded-3xl'} overflow-hidden bg-[#0d1411] shadow-xl border-2 transition-colors duration-300 flex items-center justify-center w-full h-auto max-w-full max-h-full aspect-video ${isSpeaking ? 'border-green-500 shadow-[0_0_15px_rgba(34,197,94,0.5)]' : 'border-green-900/50'}`}>
      ...
      {isVideoOff ? (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#0d1411] z-10">
          <div className={`${isFilmstrip ? 'w-8 h-8 text-[10px] mb-0.5' : 'w-16 h-16 md:w-20 md:h-20 text-xl md:text-2xl mb-4'} bg-green-900/40 rounded-full flex items-center justify-center font-bold text-green-600 shadow-inner`}>
            Y
          </div>
      ...
      <div className={`absolute ${isFilmstrip ? 'bottom-1.5 left-1.5' : 'bottom-3 left-3'} flex items-center space-x-2 z-20`}>
        <div className={`bg-green-600/90 backdrop-blur-sm ${isFilmstrip ? 'px-1.5 py-0.5 rounded text-[9px]' : 'px-2 py-1 rounded-lg text-xs'} font-medium shadow-lg flex items-center space-x-2 text-white`}>
          <span>You</span>
        </div>
        {isMuted ? (
